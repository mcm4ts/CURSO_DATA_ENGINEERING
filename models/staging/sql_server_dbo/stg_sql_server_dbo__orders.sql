...
with src as (
    select
        ...
        -- Reemplazamos nulos en shipping_service por 'no_shipping_service'
        coalesce(shipping_service, 'no_shipping_service') as shipping_service,
        -- Reemplazamos nulos en promo_id por 'NO_PROMO'
...
clean as (
    select
        ...
        to_decimal(order_total_dollars, 12, 2) as order_total_usd,
        to_decimal(order_cost_dollars, 12, 2) as order_cost_usd,
        to_decimal(shipping_cost, 12, 2) as shipping_cost_usd,
        -- Limpiamos promo_id
        md5(lower(trim(replace(coalesce(promo_id, 'no_promo'), '-', '_')))) as promo_id,
        -- Limpiamos order_status_id
