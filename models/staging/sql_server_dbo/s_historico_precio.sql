...
with src as (
  select
    ...
    -- Control de valores NaN, infinitos y nulos
    iff(price is not null
        and try_to_double(price) is not null
        and price not in ('NaN', 'inf', '-inf'),
        to_decimal(price,12,2),
        null
    ) as price_usd,

    -- Convertimos el timestamp TZ → NTZ
