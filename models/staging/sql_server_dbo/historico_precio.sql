{{ config(materialized='incremental', incremental_strategy='merge', unique_key=['product_id','valid_from']) }}

with src as (
  select
    product_id::varchar                       as product_id,
    try_to_decimal(price,12,2)::float         as price_usd,
    _fivetran_synced                          as change_ts
  from {{ source('SQL_SERVER_DBO','PRODUCTS') }}
  where coalesce(_fivetran_deleted,false)=false
  {% if is_incremental() %}
    and _fivetran_synced > (select coalesce(max(_version_synced),'1900-01-01') from {{ this }})
  {% endif %}
),
ordered as (
  select
...
change_points as (
  select product_id, price_usd, change_ts as valid_from
  from ordered
